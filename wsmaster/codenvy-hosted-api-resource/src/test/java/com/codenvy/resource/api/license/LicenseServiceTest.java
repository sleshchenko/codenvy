...
public class LicenseServiceTest {
    ...
    public void shouldGetLicense() throws Exception {
        ...
        final LicenseDto toFetch = DtoFactory.newDto(LicenseDto.class)
                                             ...
                                             .withTotalResources(Collections.singletonList(testResource));

        //when
        when(licenseManager.getByAccount(eq("account123"))).thenReturn(new LicenseImpl(toFetch));
        ...
        final LicenseDto fetchedLicense = DtoFactory.getInstance().createDtoFromJson(response.body().print(), LicenseDto.class);
        assertEquals(fetchedLicense, toFetch);
        verify(licenseManager).getByAccount("account123");
    }
}
