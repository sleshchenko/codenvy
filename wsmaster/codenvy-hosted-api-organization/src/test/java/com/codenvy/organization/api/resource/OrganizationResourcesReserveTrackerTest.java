...
import static com.codenvy.organization.api.resource.OrganizationResourcesReserveTracker.LIMITS_PER_PAGE;
...
public class OrganizationResourcesReserveTrackerTest {
    ...
    @Mock
    private Page<OrganizationResourcesLimit> limitsPage;
    @Mock
    private Page.PageRef                     nextPageRef;
    @Mock
    private OrganizationResourcesLimit       resourcesLimit;
    ...
    public void setUp() throws Exception {
        when(nextPageRef.getPageSize()).thenReturn(LIMITS_PER_PAGE);
        when(nextPageRef.getItemsBefore()).thenReturn(1L);

        when(limitsPage.getNextPageRef()).thenReturn(nextPageRef);
        when(limitsPage.hasNextPage()).thenReturn(true)
                                      .thenReturn(false);
        when(limitsPage.getItems()).thenReturn(singletonList(resourcesLimit));

        doReturn(limitsPage).when(organizationResourcesManager).getResourcesLimits(any(), anyInt(), anyLong());
    }

    @Test
    public void shouldReturnSumOfSuborganizationsResourcesLimitsWhenGettingReservedResources() throws Exception {
        //given
        ...
        doReturn(singletonList(ramResource1))
                .doReturn(asList(ramResource2,
                                 workspacesResource))
                .when(resourcesLimit).getResources();
        ...
        verify(organizationResourcesManager).getResourcesLimits("organization123", LIMITS_PER_PAGE, 0);
        verify(organizationResourcesManager).getResourcesLimits("organization123", LIMITS_PER_PAGE, 1);
